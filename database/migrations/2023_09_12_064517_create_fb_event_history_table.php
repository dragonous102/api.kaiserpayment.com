...
class CreateFbEventHistoryTable extends Migration
{
  public function up()
  {
    Schema::create('fb_event_history', function (Blueprint $table) {
      ...
      $table->string('type')->comment('The type of this action (e.g., address_required, deposit, withdrawal)');
      $table->string('status')->comment('Payment status of this transaction (e.g., pending, complete)');
      $table->string('result')->comment('The result of this action (e.g., success, fail)');
      $table->string('reason')->nullable()->comment('The reason for a failed action');
  ...
  {
    Schema::dropIfExists('fb_event_history');
  }
}
